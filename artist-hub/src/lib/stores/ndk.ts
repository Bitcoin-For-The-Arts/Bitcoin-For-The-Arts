...
export async function ensureNdk(opts?: { withSigner?: boolean }): Promise<NDK> {
  if (!browser) throw new Error('Nostr client can only run in the browser');

  const existing = get(ndk);
  if (existing) {
    if (opts?.withSigner && !existing.signer) existing.signer = new NDKNip07Signer();
    return existing;
  }
  ...
  connectPromise = (async () => {
    ...
    const client = new NDK({
      ...
      signer: opts?.withSigner ? new NDKNip07Signer() : undefined,
    });
